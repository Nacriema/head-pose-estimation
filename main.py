...
def run():
    ...
    # Init tracker
    tracker = Sort(frame_width, frame_height) 
    ...
    while True:
        ...
        if len(faces) > 0:
            ...
            # NOTE: Check the refined faces - faces has been well aligned. In this case, we always have the detected faces
            print(f'Refined faces: {refined_faces}, shape: {refined_faces.shape}')  # Shape (n, 5) and n >= 1

            # NOTE: Update SORT
            refined_faces, estimators = tracker.update(refined_faces)
            print(f'Track bbs ids: {refined_faces}')  # track_bbs_ids: [[x1, y1, x2, y2 track_ID]]

            if len(refined_faces):
                for face, estimator in zip(refined_faces, estimators):
            ...
            face_detector.visualize(frame, refined_faces)
        else:
            # NOTE: No face detected, still update the tracker
            tracker.update()  # Use default - empty numpy array
